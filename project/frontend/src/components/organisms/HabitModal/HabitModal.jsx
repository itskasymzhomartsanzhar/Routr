...
const HabitModal = ({
  ...
  isPremium = false,
  canCreatePublicHabit = true,
  publicJoinOnly = false,
  maxPublicHabits = null,
  publicHabitsCount = 0
}) => {
  ...
  const reminderLimit = isPremium ? 5 : 3
  const canAddReminder = habitData.reminderTimes.length < reminderLimit
  const getCategoryName = (id) => categories.find((item) => item.id === id)?.name || 'Личное'
  const isEditingPublicHabit = mode === 'edit' && initialData?.visibility === 'Публичный'
  const canSelectPublic = isEditingPublicHabit || canCreatePublicHabit
  const publicVisibilityHint = publicJoinOnly
    ? 'На текущем уровне можно только присоединяться к публичным привычкам.'
    : maxPublicHabits != null && !canCreatePublicHabit
      ? `Достигнут лимит публичных привычек: ${publicHabitsCount}/${maxPublicHabits}.`
      : null
  ...
  useEffect(() => {
    ...
    if (mode === 'edit' && initialData) {
      ...
      setHabitData({
        ...
        categoryId: initialData.categoryId ?? defaultHabitData.categoryId,
  ...
  useEffect(() => {
    if (!isOpen) return
    if (!habitData.categoryId && defaultCategoryId) {
      setHabitData((prev) => ({ ...prev, categoryId: defaultCategoryId }))
    }
  }, [isOpen, defaultCategoryId, habitData.categoryId])
  ...
  useEffect(() => {
    if (!isOpen) return
    if (habitData.visibility === 'Публичный' && !canSelectPublic) {
      setHabitData((prev) => ({ ...prev, visibility: 'Приватный' }))
    }
        ...
        <div className="add-habit-modal__scrollable">
          ...
          <div className="add-habit-modal__section">
            ...
            <div className="add-habit-modal__dropdown" ref={categoryRef}>
              ...
              >
                <span>{getCategoryName(habitData.categoryId)}</span>
                ...
                <div className="add-habit-modal__dropdown-menu">
                  {categories.map((option) => (
                    <button
                      key={option.id}
                      type="button"
                      className={`add-habit-modal__dropdown-item ${habitData.categoryId === option.id ? 'add-habit-modal__dropdown-item--active' : ''}`}
                      onClick={() => {
                        setHabitData({ ...habitData, categoryId: option.id })
                        setOpenDropdown(null)
                      }}
                    >
                      <span>{option.name}</span>
                      {habitData.categoryId === option.id && (
            ...
            <div className="add-habit-modal__reminder">
              ...
              {habitData.reminderTimes.map((timeValue, index) => (
                <div key={`reminder-${index}`} className="add-habit-modal__time">
                  ...
                  <path fillRule="evenodd" clipRule="evenodd" d="M14.6665 7.99992C14.6665 11.6826 11.6819 14.6666 7.99986 14.6666C4.31786 14.6666 1.33319 11.6826 1.33319 7.99992C1.33319 4.31859 4.31786 1.33325 7.99986 1.33325C11.6819 1.33325 14.6665 4.31859 14.6665 7.99992Z" fill="white"/>
                  <path fillRule="evenodd" clipRule="evenodd" d="M10.3824 10.5429C10.295 10.5429 10.207 10.5203 10.1264 10.4729L7.50902 8.91161C7.35835 8.82094 7.26569 8.65761 7.26569 8.48161V5.11694C7.26569 4.84094 7.48969 4.61694 7.76569 4.61694C8.04169 4.61694 8.26569 4.84094 8.26569 5.11694V8.19761L10.639 9.61294C10.8757 9.75494 10.9537 10.0616 10.8124 10.2989C10.7184 10.4556 10.5524 10.5429 10.3824 10.5429Z" fill="#3843FF"/>
                  </svg>
...
<path d="M1.875 11.25C1.53125 11.25 1.23698 11.1276 0.992187 10.8828C0.747396 10.638 0.625 10.3438 0.625 10V1.875H0V0.625H3.125V0H6.875V0.625H10V1.875H9.375V10C9.375 10.3438 9.2526 10.638 9.00781 10.8828C8.76302 11.1276 8.46875 11.25 8.125 11.25H1.875ZM3.125 8.75H4.375V3.125H3.125V8.75ZM5.625 8.75H6.875V3.125H5.625V8.75Z" fill="#707579"/>
            ...
            {canAddReminder && (
              ...
              >
                Добавить напоминание ({habitData.reminderTimes.length}/{reminderLimit})
              </button>
            )}
          </div>
          ...
          <div className="add-habit-modal__section">
            ...
            >
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setHabitData({ ...habitData, visibility: 'Публичный' })
                }}
                type="button"
            ...
            {publicVisibilityHint && (
              <p className="add-habit-modal__hint">{publicVisibilityHint}</p>
            )}
          </div>
          </div>
        </div>
